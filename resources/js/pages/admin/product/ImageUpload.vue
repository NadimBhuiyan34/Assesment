...
<template>
    ...
    <AppLayout :breadcrumbs="[
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-10">
            <div v-for="image in props.product.images" :key="image.id"
                class="group relative bg-white rounded-xl shadow overflow-hidden hover:shadow-lg transition">
                <img :src="`/storage/${image.image_path}`" :alt="props.product.name"
                ...
                <!-- Featured Badge -->
                <div v-if="image.is_featured"
                    class="absolute top-3 left-3 bg-indigo-600 text-white text-xs px-3 py-1 rounded-full shadow">
                    🌟 Featured
                </div>

                <!-- Delete Button -->
                <Link as="button" :href="`/product_images/${image.id}`" @click.prevent="deleteImage(image.id)"
